...
public class CpuScheduler {
	...
	public Job roundRobin(int[] a, int[] p, LinkedList<Job> jobTable) {
		...
		if(!(runJob.getMaxCpuTime()<=runJob.getCpuTimeUsed())) {
			...
			if(runJob.getMaxCpuTime() < timeQuantum+runJob.getCpuTimeUsed() && !runJob.isBlocked() && runJob.isInMemory()) {
			...
			else if(runJob.getMaxCpuTime() >= timeQuantum+runJob.getCpuTimeUsed() && !runJob.isBlocked() && runJob.isInMemory()) {
